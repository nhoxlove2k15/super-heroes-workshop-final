...
//        results.map(fight -> stats.add(fight)).invoke();
        return results
            .onItem().transform(stats::add)
            .invoke(() -> {
                logger.info("Fight received. Computed the team statistics");
                System.out.println("Fight received. Computed the team statistics");
            });
    }
